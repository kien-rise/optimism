...
func (g *GameCallerCreator) CreateContract(ctx context.Context, game gameTypes.GameMetadata) (GameCaller, error) {
	...
	case faultTypes.CannonGameType,
		...
		return fdg, nil
	case faultTypes.OPSuccinctFPGameType:
		fdg, err := contracts.NewOPSuccinctFaultDisputeGame(g.m, game.Proxy, g.caller)
		if err != nil {
			return nil, fmt.Errorf("failed to create OP Succinct fault dispute game contract: %w", err)
		}
		g.cache.Add(game.Proxy, fdg)
		return fdg, nil
	default:
